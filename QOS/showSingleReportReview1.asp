...
'	end if
		...
		SOP_P=""
		SOP_W=""
		getR_Id = Request.QueryString("MID")
		'response.write getR_Id
		SQL = "select * from Tb_Review where M_Code='"&getR_Id&"'"
		set RecReview = Server.CreateObject("ADODB.RECORDSET")
		RecReview.open SQL,ConQS,1,3
		if RecReview.RecordCount = 0 then
		response.write "<script lnaguage=""javascript"" >"
		response.write "alert('Data not found ! \r\n Try again!');"
		response.write "window.location.href='http://filing.fda.moph.go.th/kmfda/_block/qos/FReview.asp'; "
