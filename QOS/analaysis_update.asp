...
5. พัฒนาการบริหารจัดการองค์กรสู่ความเป็นเลิศ</label></td>
  ...
  <tr>
    <td><table width="100%" border="0" cellspacing="0" cellpadding="5">
      ...
      <tr>
      <td>&nbsp;</td>
      <td>คะแนนรวม : <input type="text"  name="txtSumAll" id="txtSumAll" readonly width="20" value="<%=getAnalis_Sum%>"/></td>
      </tr>
    </table></td>
  </tr>
  <tr>
  ...
  <tr>
  <td><input type="button"  value="บันทึกข้อมูล"  onclick="AnalisCheckSaveUpdate()" />&nbsp;&nbsp;&nbsp;<input type="button" value="กลับหน้ากรอกข้อมูล" onClick="javascript:{window.open('http://filing.fda.moph.go.th/kmfda/_block/qos/analaysis.asp','_self')}" /> <%  if isEmpty(getMID) = False then response.write "<input type=""button""  value=""ดูหน้ารายงาน"" onclick=""openReport()""  />" end if  %></td>
  </tr>
</table>
</form>
</body>
