...
if isEmpty(getID) = true then
	getID = Request.Form("hidid")
end if
if isEmpty(getSource) = true then
getSource = Request.Form("hidsource")
end if

'  #----------------------------------------------Start Block for update Flag_Check in DB------------------------------------#
if getSave = "Save" then
sql = "Update Tb_ManagementReview set Flag_Check=True where  MR_ID="&getID

sqlLog = "Insert into Tb_ManagementReviewLog (D_Id,UserName,IP,Log_Date,Log_Method,MR_ID) values ('"&getDID&"','"&session("member")&"','"&Request.ServerVariables("REMOTE_ADDR")&"','"&Datemmddyyyy&"','QMR Check ','"&getID&"')"
...
'  #---------------------------------------------End Block for update Flag_Check in DB--------------------------------------#
...
if isEmpty(getID)  = true or isEmpty(getDID) = true then
	'Response.Redirect("http://filing.fda.moph.go.th/kmfda/_block/qos/")
	Response.write "<script>"
	Response.write "	alert('ท่านไม่ได้รับสิทธิ์ในการเข้าดูระบบนี้'); "
	Response.write " 	window.location.href=""default.asp""; "
...
<body bgcolor="#ffffff">
<form name="frmCheckManagementReview" id="frmCheckManagementReview" enctype="application/x-www-form-urlencoded" method="post" action="Check_ManagementReview.asp">
<input type="hidden" name="hidid" id="hidid" value="<%=getID%>" />
<input type="hidden" name="hidDid" id="hidDid" value="<%=getDID%>" />
<input type="hidden" name="hidsource" id="hidsource" value="<%=getSource%>" />
...
<% if isEmpty(getSave) = True or getSave <> "Save" then %>
      <input type="button" name="butSave" id="butSave" value="ยอมรับรายงาน"  onclick="go_UpdateCheck()"  />
<% else %> 
	  <input type="button" name="butPrint" id="butPrint" value="พิมพ์เอกสาร"  onclick="javascript:{window.print();}"  />
<% end if %>
      &nbsp;&nbsp;
